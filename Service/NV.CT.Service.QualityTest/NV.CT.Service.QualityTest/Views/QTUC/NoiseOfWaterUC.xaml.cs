...
{
    /// <summary>
    /// NoiseOfWaterUC.xaml 的交互逻辑
    /// </summary>
    public partial class NoiseOfWaterUC
    {
        /// <summary>
        /// 控件和对应ROI框的字典缓存
        /// </summary>
        ...
        public NoiseOfWaterUC(IMessagePrintService printLog, IIntegrationPhantomService integrationPhantomService)
        {
            InitializeComponent();
            _printLog = printLog;
        ...
        public override async Task<ResultModel> AfterRecon(ItemEntryModel model)
        {
            ...
            for (var i = 0; i < ics.Length; i++)
            {
                ...
                var radius = imageData.ImageWidth * 0.2;
        ...
        private void Calculate(ItemEntryModel model, SeriesImageControl ic)
        {
            ...
            var noiseValue = anno.MeasureResults.ResultList.FirstOrDefault(x => x.ResultName == MyEllipseCTCalculator.Noise)?.Content ?? 0;
            SetMeasuredValue(model.Param, index, noiseValue);
        }
