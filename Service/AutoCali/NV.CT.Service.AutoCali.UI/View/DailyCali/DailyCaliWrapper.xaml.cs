...
{
    /// <summary>
    /// DailyCaliWrapper.xaml 的交互逻辑
    /// </summary>
    public partial class DailyCaliWrapper : UserControl, IServiceControl
    {
        private static readonly string ClassName = nameof(DailyCaliWrapper);
        private static readonly string MethodName_GetTipOnClosing = nameof(GetTipOnClosing);

        public DailyCaliWrapper()
        {
        ...
        public static readonly string Service_App_Name = "DailyCalibration";
        ...
        {
            string block = $"{ClassName}.{MethodName_GetTipOnClosing}";
        ...
        private void UserControl_Loaded(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            //不要在xaml中就创建ViewModel，并设置DataContext
            if (_isLoaded) return;

            string block = $"{ClassName}.UserControl_Loaded";
            LogService.Instance.Debug(ServiceCategory.AutoCali, $"Beginning {block}");

            _caliViewModel = new DailyCaliViewModel();
        ...
        {
            //string block = $"{ClassName}.UserControl_Unloaded";
            //LogService.Instance.Debug(ServiceCategory.AutoCali, $"Beginning {block}");
