...
using NV.CT.Service.HardwareTest.UserControls.Universal.Interfaces;
...
{
    public partial class ImageViewerManager : ObservableObject
    {
        public ImageViewerManager() 
        {
        ...
        private void InitializeProperties() 
        {
            ...
            {
                new ImageViewerMutexAction(ImageViewerActionType.Zoom, false, PackIconKind.MagnifyPlusOutline, "Zoom"),
                new ImageViewerMutexAction(ImageViewerActionType.Drag, false, PackIconKind.DragVariant, "Drag"),
                new ImageViewerMutexAction(ImageViewerActionType.Rotate, false, PackIconKind.CropRotate, "Rotate"),
                new ImageViewerMutexAction(ImageViewerActionType.Rectangle, false, PackIconKind.VectorSquareClose, "Rectangle ROI"),
                new ImageViewerMutexAction(ImageViewerActionType.Circle, false, PackIconKind.DotsCircle, "Circle ROI"),
                new ImageViewerMutexAction(ImageViewerActionType.RectangleWWWL, false, PackIconKind.ShapeRectanglePlus, "Rectangle WWWL"),
                new ImageViewerNormalAction(ImageViewerActionType.AutoWWWL, false, PackIconKind.AutoFix, "Auto WWWL"),
                new ImageViewerNormalAction(ImageViewerActionType.OneToOne, false, PackIconKind.Numeric1BoxMultipleOutline, "1:1 Bitmap"),
                new ImageViewerNormalAction(ImageViewerActionType.AutoFit, false, PackIconKind.BorderHorizontal, "Auto Fit"),
                new ImageViewerNormalAction(ImageViewerActionType.ResetView, false, PackIconKind.Replay, "Reset View"),
                new ImageViewerNormalAction(ImageViewerActionType.ClearView, false, PackIconKind.DeleteSweepOutline, "Clear View")
            };
        }
        ...
        /// 当前生数据信息
        /// </summary>
        public RawDataInfo? CurrentRawDataInfo { set; get; }
        /// <summary>
        /// 当前扫描序列
        /// </summary>
        public ScanSeries? CurrentScanSeries { set; get; } = null!;
        ...
        public bool IsDataLoaded => RawDataPool is not null && RawDataPool.Count() > 0;
        ...
        {
            WeakReferenceMessenger.Default.Register<ScrollBarThumbChangedMessage>(this, ImageContentControlScrollBarThumbChanged);
            WeakReferenceMessenger.Default.Register<ImageChainImageSortedMessage>(this, ImageChainImageSortedCompleted);
            WeakReferenceMessenger.Default.Register<RawDataSetSortedMessage>(this, RawDataImageSortedCompleted);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public GenericResponse<bool> CutImage(bool useLocalCutXmlConfigFile = false)
        {
            //执行ImageCut
            var response = RawDataReadWriteHelper.Instance.CutImage(RawDataCutProgressChanging, useLocalCutXmlConfigFile);
            //校验
            if (!response.status)
            {
            ...
            //显示ImageCut数据
            LoadRawDataListCommon(response.data);

            return new(true, "Image after cut has been loaded.");
        }
        ...
        private void ImageChainImageSortedCompleted(object recipient, ImageChainImageSortedMessage message)
        {
            //记录排序后的生数据
        ...
        private void RawDataImageSortedCompleted(object recipient, RawDataSetSortedMessage message)
        {
            //记录排序后的生数据
            RawDataPool = message.sortedRawDataSet;
            //显示排序后的生数据
            ImageViewer.SortRawDataList(RawDataPool.Select(t => t.Id));
            //重置Index
            CurrentImageIndex = 1;
        ...
        public void LoadRawDataList(RawDataReadResult rawDataReadResult) 
        {
            CurrentRawDataInfo = rawDataReadResult.RawDataInfo;

            LoadRawDataListCommon(rawDataReadResult.ScanSeries!);
        }

        private void LoadRawDataListCommon(ScanSeries scanSeries) 
        {
            CurrentScanSeries = scanSeries;
            //清空ImageViewer
            ClearImageViewer();
            //记录加载数据类型
            ...
            //加载数据
            ImageViewer.LoadRawDataList(imageRawDataInfos, scanSeries!.DetectorMaps.ToImageViewerDetectorMaps().ToList());
            //更新显示
            CurrentImageIndex = 1;
        ...
        public void LoadDicomSeries(string directory) 
        {
            ...
            //更新显示
            CurrentImageIndex = 1;
            LoadedImageCount = GetLocalDirectoryDicomCount(directory);
        }

        private int GetLocalDirectoryDicomCount(string directory) 
        {
            var fileNames = Directory.GetFiles(directory, "*.dcm");
