...
namespace NV.CT.JobService.JobExecutor
{
    public class ImportByRawDataExecutor : IImportTaskExecutor
    {
        ...
        public string PatientNameListString => _patientNameListString;
        ...
        private void InitFilesList()
        {
            ...
            {
                _logger.LogError($"[ImportByRawDataExecutor] The source folder does not exist:{_sourceRootPath}");
        ...
        private  void ActivateImport()
        {
            ...
            try
            {
                ...
                //发送导入结束通知
                _logger.LogInformation($"[ImportByRawDataExecutor] finished for {_sourceRootPath}, progress is:{ProcessedCount}/{TotalCount}");
            ...
            {
                _logger.LogWarning($"ImportByRawDataExecutor is cancelled for [{_sourceRootPath}], the exception is:{canceledException.Message}");
            ...
            {
                _logger.LogError($"[ImportByRawDataExecutor]:Failed to import from source path:{_sourceRootPath}, the error message is:{ex.Message}");
        ...
        private void CheckIfAskedToCancel()
        {
            ...
            if (_cancellationTokenSource.Token.IsCancellationRequested)
            {
                ...
                _logger.LogTrace("ExportToRawDataExecutor received cancellation request.");
