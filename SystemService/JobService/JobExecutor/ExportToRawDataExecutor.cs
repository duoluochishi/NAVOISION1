...
namespace NV.CT.JobService.JobExecutor
{
    public class ExportToRawDataExecutor : ITaskExecutor
    {
        ...
        private void ActivateExport()
        {
            ...
            {
                _logger.LogWarning($"ExportToRawDataExecutor is cancelled for [{_patientNameListString}], the exception is: {canceledException.Message}");
        ...
        private void CopyToDisk()
        {
            ...
            foreach (var srcPath in SrcPaths)
            {
                ...
                _rawDataService.UpdateExportStatusById(RawDataIDList[index], true);
                index++;
